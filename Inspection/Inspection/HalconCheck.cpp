...
void InitSurCheck (HTuple hv_MessageHandle, HTuple hv_Width, HTuple hv_Height);
...
void Init_SurClassCheck (HTuple hv_CurrentScheme, HTuple hv_lsCheckParam, HTuple hv_MessageHandle);
void CalculateFeatures_SurfaceNoClass (HObject ho_InputRegion, HObject ho_Image, 
    HObject ho_DynThreshRegion1, HObject ho_DynThreshRegion2, HObject ho_DynThreshRegion3, 
    HObject ho_DynThreshRegion4, HTuple hv_ProdR1, HTuple hv_ProdR2, HTuple hv_ProdC1, 
    HTuple hv_ProdC2, HTuple hv_ErrTypeIndex, HTuple hv_MessageHandle, HTuple *hv_Score, 
    HTupleVector/*{eTupleVector,Dim=1}*/ *hvec_FeatureVals);
void Floor_Detection (HObject ho_PreprocessImage, HTuple hv_ShowHide, HTuple hv_WindowHandle, 
    HTuple hv_MessageHandle, HTuple *hv_outResultInfo, HTuple *hv_outWindowInfo, 
    HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_outFeatureVals);
void GetPyFeature2 (HObject ho_ImageResult, HTuple hv_MessageHandle, HTuple hv_ShowHide, 
    HTuple hv_WindowHandle, HTuple hv_PyResultVal, HTuple hv_R, HTuple hv_C, HTuple *hv_ResultInfo, 
    HTuple *hv_WindowInfo, HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_FeatureVals);
    ...
    HTuple *hv_MessageHandle)
{
  ...
  HTuple  hv_CheckParam, hv_Position, hv_DefectNames, hv_DefectVals;
  ...
  HTuple  hv_Resolution_W, hv_resolutionratio, hv_val, hv_ProductSetting;
  HTuple  hv_DiameterVal, hv_Diameter_offset, hv_DiameterMinMax;
  HTuple  hv_GuidepinVal, hv_Guidepin_offset, hv_GuidepinMinMax;
  HTuple  hv_GuidepinHeightoffset, hv_ProductHeight, hv_OCRSigns;
  HTuple  hv_Index1, hv_OCRval, hv_OCRDirection, hv_val1;
  HTuple  hv_lsCheckParam;
  ...
  TupleSplit(hv_BaseName, "_", &hv_CheckParam);
  ...
  load_config_set_message_Number((*hv_MessageHandle), hv_CurrentScheme, "基本参数", 
      "瑕疵小图宽", "ErrImgSaveWidth", 100, &hv_val);
  load_config_set_message_Number((*hv_MessageHandle), hv_CurrentScheme, "基本参数", 
      "瑕疵小图高", "ErrImgSaveHeight", 100, &hv_val);

  //[ProductSetting][产品规格]
  hv_ProductSetting = hv_DirName+"/SystemConfiguration/ProductSetting.ini";
  load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
      "直径", "DiameterVal", 18, &hv_DiameterVal);

  load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
      "直径容差", "Diameter_offset", 0.5, &hv_Diameter_offset);
  ...
  load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
      "导针间距", "GuidepinVal", 0.5, &hv_GuidepinVal);
  load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
      "导针间距容差", "Guidepin_offset", 0.5, &hv_Guidepin_offset);
  ...
  load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
      "导针高度容差", "GuidepinHeightoffset", 0.5, &hv_GuidepinHeightoffset);

  load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
      "产品高度", "ProductHeight", 25, &hv_ProductHeight);
  load_config_split(hv_ProductSetting, "产品规格", "OCRSigns", "uF|V", "|", &hv_OCRSigns);
  SetMessageTuple((*hv_MessageHandle), "OCRSigns", hv_OCRSigns);
  {
  HTuple end_val92 = (hv_OCRSigns.TupleLength())-1;
  HTuple step_val92 = 1;
  for (hv_Index1=0; hv_Index1.Continue(end_val92, step_val92); hv_Index1 += step_val92)
  {
    load_config_set_message_Str((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
        HTuple(hv_OCRSigns[hv_Index1])+"值", HTuple(hv_OCRSigns[hv_Index1])+"_val", 
        "120", &hv_OCRval);
    //方向(0,90,180,270)
    load_config_set_message_Number((*hv_MessageHandle), hv_ProductSetting, "产品规格", 
        HTuple(hv_OCRSigns[hv_Index1])+"方向", HTuple(hv_OCRSigns[hv_Index1])+"_Direction", 
        0, &hv_OCRDirection);
    load_config_number(hv_ProductSetting, "产品规格", HTuple(hv_OCRSigns[hv_Index1])+"_亮黑", 
        0, &hv_val1);
    if (0 != (hv_val1==0))
    {
      SetMessageTuple((*hv_MessageHandle), "TextModel_polarity", "light_on_dark");
    }
    else
    {
      SetMessageTuple((*hv_MessageHandle), "TextModel_polarity", "dark_on_light");
    }
  }
  }
  ...
  {
    InitSurCheck((*hv_MessageHandle), hv_ImageW, hv_ImageH);
  }
  else if (0 != (HTuple(hv_lsCheckParam[0])==HTuple("分类检测")))
  {
    Init_SurClassCheck(hv_CurrentScheme, hv_lsCheckParam, (*hv_MessageHandle));
  }
...
void CloseCheck (HTuple hv_MessageInHandle)
{
  ...
  // Local control variables
  HTuple  hv_message_keys, hv_Selection, hv_DrawID;
  HTuple  hv_Index, hv_Selection2, hv_key_exist, hv_TupleData;
  HTuple  hv_TupleData1;
  ...
  //
  CreateDrawingObjectCircle(100, 100, 80, &hv_DrawID);

  {
  HTuple end_val7 = (hv_Selection.TupleLength())-1;
  HTuple step_val7 = 1;
  for (hv_Index=0; hv_Index.Continue(end_val7, step_val7); hv_Index += step_val7)
  {
    ...
    if (0 != (hv_key_exist==1))
    {
      ...
      else if (0 != (hv_TupleData==HTuple("clear_drawing_object")))
      {
        GetMessageTuple(hv_MessageInHandle, hv_Selection2, &hv_DrawID);
        ClearDrawingObject(hv_DrawID);

      }
    }
  }
  }
  SetMessageParam(hv_MessageInHandle, "remove_all_keys", HTuple(), HTuple());
    ...
    HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_FeatureVals)
{
  ...
  // Local control variables
  HTuple  hv_lsClassResult, hv_CheckParam, hv_lsCheckParam;
  HTuple  hv_Width, hv_Height, hv_outResultInfo, hv_outWindowInfo;
  ...
  if (0 != (HTuple(hv_lsCheckParam[0])==HTuple("字符和瑕疵检测")))
  {
    ...
    AttachBackgroundToWindow((*ho_PreprocessImage), hv_WindowHandle);
  ...
  {
	  int indexi=0;
	  if(hv_lsCheckParam[1]=="1")
	  {
		  indexi=1;
	  }
	  else if(hv_lsCheckParam[1]=="2")
	  {
		  indexi=2;
	  }
	  else  if(hv_lsCheckParam[1]=="3")
	  {
		  indexi=3;
	  }
	  else  if(hv_lsCheckParam[1]=="4")
	  {
		  indexi=4;
	  }
	  int fsdasads=GetTickCount();
	...
	fsdasads =GetTickCount()-fsdasads;
	if(fsdasads>ddddd[indexi])
		ddddd[indexi]=fsdasads;
  ...
  {
    Floor_Detection((*ho_PreprocessImage), hv_ShowHide, hv_WindowHandle, hv_MessageHandle, 
        &hv_outResultInfo, &hv_outWindowInfo, &hvec_outFeatureVals);
  }
  else if (0 != (HTuple(hv_lsCheckParam[0])==HTuple("底面检测")))
  {
    ...
    HTuple hv_MessageHandle, HTuple *hv_ResultInfo, HTuple *hv_WindowInfo, HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_FeatureVals)
{
  ...
  // Local iconic variables
  HObject  ho_EmptyRegion, ho_RegionLines1, ho_RegionLines2;
  HObject  ho_CheckRegion, ho_ImgReduced, ho_ProductRegion;
  HObject  ho_RegionReduced, ho_Mean1, ho_Mean2, ho_AlreadySeg;
  HObject  ho_DynThreshRegion, ho_DynThreshRegionALL, ho_DynThreshRegion1;
  HObject  ho_RegionInput, ho_DynThreshRegion2, ho_DynThreshRegion3;
  HObject  ho_DynThreshRegion4, ho_DynThreshRegionALL_Temp;
  HObject  ho_ObjForAsses;

  // Local control variables
  HTuple  hv_AREA_MAX, hv_ImgLeftOffset1, hv_ImgRightOffset1;
  HTuple  hv_RectDrawID, hv_ImgLeftOffset, hv_ImgRightOffset;
  HTuple  hv_CurrentScheme, hv_str_DynMode1, hv_str_DynMode2;
  HTuple  hv_str_DynMode3, hv_str_DynMode4, hv_Surf_MeanW1;
  HTuple  hv_Surf_MeanH1, hv_Surf_MeanW2, hv_Surf_MeanH2;
  HTuple  hv_Surf_ProdGray_Low, hv_Surf_ProdGray_High, hv_Surf_CloseW;
  HTuple  hv_Surf_CloseH, hv_Surf_ProdArea, hv_Surf_EroseW;
  HTuple  hv_Surf_EroseH, hv_Surf_DynThresh1, hv_Surf_Dyn1CloseW;
  HTuple  hv_Surf_Dyn1CloseH, hv_Surf_Dyn1SizePixS, hv_Surf_DynThresh2;
  HTuple  hv_Surf_Dyn2CloseW, hv_Surf_Dyn2CloseH, hv_Surf_Dyn2SizePixS;
  HTuple  hv_Surf_DynThresh3, hv_Surf_Dyn3CloseW, hv_Surf_Dyn3CloseH;
  HTuple  hv_Surf_Dyn3SizePixS, hv_Surf_DynThresh4, hv_Surf_Dyn4CloseW;
  HTuple  hv_Surf_Dyn4CloseH, hv_Surf_Dyn4SizePixS, hv_MaxReturnNum;
  HTuple  hv_Width, hv_Height, hv_Area, hv_ImageCenterRow;
  HTuple  hv_ImageCenterCol, hv_IsEmpty, hv_Row1, hv_Column1;
  HTuple  hv_Row2, hv_Column2, hv_Exception, hv_ProdR1, hv_ProdC1;
  HTuple  hv_ProdR2, hv_ProdC2, hv_ImageAsses_Width, hv_ImageAsses_Height;
  HTuple  hv_tempThreshArea, hv_NULL, hv_ErrNumber1, hv_ErrNumber2;
  HTuple  hv_ErrNumber3, hv_ErrNumber4, hv_Row, hv_Column;
  HTuple  hv_Indices, hv_AreaArray, hv_Number, hv_Index, hv_ErrTypeIndex;
  HTuple  hv_Score;
  ...
  GetMessageTuple(hv_MessageHandle, "ImgLeftOffset", &hv_ImgLeftOffset1);
  GetMessageTuple(hv_MessageHandle, "ImgRightOffset", &hv_ImgRightOffset1);
  GetMessageTuple(hv_MessageHandle, "RectDrawID", &hv_RectDrawID);
  //attach_drawing_object_to_window (WindowHandle, RectDrawID)
  GetDrawingObjectParams(hv_RectDrawID, "column1", &hv_ImgLeftOffset);
  GetDrawingObjectParams(hv_RectDrawID, "column2", &hv_ImgRightOffset);
  if (0 != (HTuple(hv_ImgLeftOffset1!=hv_ImgLeftOffset).TupleOr(hv_ImgRightOffset!=hv_ImgRightOffset1)))
  {
    GetMessageTuple(hv_MessageHandle, "CurrentScheme", &hv_CurrentScheme);
    hv_ImgLeftOffset1 = hv_ImgLeftOffset;
    hv_ImgRightOffset1 = hv_ImgRightOffset;
    TupleString(hv_ImgLeftOffset1, "#", &hv_ImgLeftOffset1);
    TupleString(hv_ImgRightOffset1, "#", &hv_ImgRightOffset1);
    ExConfigIniWriteStr(hv_CurrentScheme, "检测区域", "左边界", hv_ImgLeftOffset1);
    ExConfigIniWriteStr(hv_CurrentScheme, "检测区域", "右边界", hv_ImgRightOffset1);
    SetMessageTuple(hv_MessageHandle, "ImgLeftOffset", hv_ImgLeftOffset1);
    SetMessageTuple(hv_MessageHandle, "ImgRightOffset", hv_ImgRightOffset1);
  }
  ...
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1CloseW", &hv_Surf_Dyn1CloseW);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1CloseH", &hv_Surf_Dyn1CloseH);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixS", &hv_Surf_Dyn1SizePixS);
  //动态阈值2
  GetMessageTuple(hv_MessageHandle, "Surf_DynThresh2", &hv_Surf_DynThresh2);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2CloseW", &hv_Surf_Dyn2CloseW);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2CloseH", &hv_Surf_Dyn2CloseH);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2SizePixS", &hv_Surf_Dyn2SizePixS);
  //动态阈值3
  GetMessageTuple(hv_MessageHandle, "Surf_DynThresh3", &hv_Surf_DynThresh3);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3CloseW", &hv_Surf_Dyn3CloseW);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3CloseH", &hv_Surf_Dyn3CloseH);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3SizePixS", &hv_Surf_Dyn3SizePixS);
  //动态阈值4
  GetMessageTuple(hv_MessageHandle, "Surf_DynThresh4", &hv_Surf_DynThresh4);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4CloseW", &hv_Surf_Dyn4CloseW);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4CloseH", &hv_Surf_Dyn4CloseH);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4SizePixS", &hv_Surf_Dyn4SizePixS);

  GetMessageTuple(hv_MessageHandle, "MaxReturnNum", &hv_MaxReturnNum);

  GetImageSize(ho_Image, &hv_Width, &hv_Height);

  GenRegionLine(&ho_RegionLines1, 0, hv_ImgLeftOffset, hv_Height, hv_ImgLeftOffset);
  GenRegionLine(&ho_RegionLines2, 0, hv_ImgRightOffset, hv_Height, hv_ImgRightOffset);
  DispObj(ho_RegionLines1, hv_WindowHandle);
  DispObj(ho_RegionLines2, hv_WindowHandle);

  GenRectangle1(&ho_CheckRegion, 0, hv_ImgLeftOffset, hv_Height, hv_ImgRightOffset);
  //get_message_obj (CheckRegion, MessageHandle, 'hCheckRegion')
  ...
  TestEqualRegion(ho_ProductRegion, ho_EmptyRegion, &hv_IsEmpty);
  if (0 != hv_IsEmpty)
  {
    (*hv_ResultInfo) = "OK";
    (*hv_WindowInfo) = HTuple();
  ...
  SelectShapeStd(ho_ProductRegion, &ho_ProductRegion, "max_area", 100);
  FillUp(ho_ProductRegion, &ho_ProductRegion);
  SelectShape(ho_ProductRegion, &ho_ProductRegion, "area", "and", hv_Surf_ProdArea, 
      hv_AREA_MAX);
  try
  {
    SmallestRectangle1(ho_ProductRegion, &hv_Row1, &hv_Column1, &hv_Row2, &hv_Column2);
    GenRectangle1(&ho_ProductRegion, hv_Row1, hv_Column1+hv_Surf_EroseW, hv_Row2, 
        hv_Column2-hv_Surf_EroseW);
  }
  // catch (Exception) 
  catch (HalconCpp::HException &HDevExpDefaultException)
  {
  ...
  if (0 != (1>0))
  {
    ...
    GenEmptyObj(&ho_DynThreshRegionALL);
    ...
    {
      if (0 != (HTuple(hv_Surf_Dyn1CloseW>0).TupleAnd(hv_Surf_Dyn1CloseH>0)))
      {
        ClosingRectangle1(ho_DynThreshRegion1, &ho_DynThreshRegion1, hv_Surf_Dyn1CloseW, 
            hv_Surf_Dyn1CloseH);
      }
      Connection(ho_DynThreshRegion1, &ho_DynThreshRegion1);
      SelectShape(ho_DynThreshRegion1, &ho_DynThreshRegion1, "area", "and", hv_Surf_Dyn1SizePixS, 
          hv_AREA_MAX);
      ho_RegionInput = ho_DynThreshRegion1;
      //GetRemainRegion (DynThreshRegion1, RegionReduced, AlreadySeg, ProductRegion, DynThreshRegion, DynThreshRegion1, AlreadySeg, RegionReduced, DynThreshRegion)

      CountObj(ho_DynThreshRegion1, &hv_ErrNumber1);
      ConcatObj(ho_DynThreshRegionALL, ho_DynThreshRegion1, &ho_DynThreshRegionALL
          );
    }
    else
    {
    ...
    if (0 != (hv_tempThreshArea>0))
    {
      if (0 != (HTuple(hv_Surf_Dyn2CloseW>0).TupleAnd(hv_Surf_Dyn2CloseH>0)))
      {
        ClosingRectangle1(ho_DynThreshRegion2, &ho_DynThreshRegion2, hv_Surf_Dyn2CloseW, 
            hv_Surf_Dyn2CloseH);
      }
      Connection(ho_DynThreshRegion2, &ho_DynThreshRegion2);
      SelectShape(ho_DynThreshRegion2, &ho_DynThreshRegion2, "area", "and", hv_Surf_Dyn2SizePixS, 
          hv_AREA_MAX);
      //GetRemainRegion (DynThreshRegion2, RegionReduced, AlreadySeg, ProductRegion, DynThreshRegion, DynThreshRegion2, AlreadySeg, RegionReduced, DynThreshRegion)
      Difference(ho_DynThreshRegion2, ho_DynThreshRegionALL, &ho_DynThreshRegion2
          );
      CountObj(ho_DynThreshRegion2, &hv_ErrNumber2);
      ConcatObj(ho_DynThreshRegionALL, ho_DynThreshRegion2, &ho_DynThreshRegionALL
          );
    }
    else
    {
    ...
    if (0 != (hv_tempThreshArea>0))
    {
      if (0 != (hv_Surf_Dyn3CloseW>0))
      {
        ClosingCircle(ho_DynThreshRegion3, &ho_DynThreshRegion3, hv_Surf_Dyn3CloseW);
      }
      Connection(ho_DynThreshRegion3, &ho_DynThreshRegion3);
      RemoveNoiseRegion(ho_DynThreshRegion3, &ho_DynThreshRegion3, "n_48");
      if (0 != (hv_Surf_Dyn3CloseH>0))
      {
        ClosingCircle(ho_DynThreshRegion3, &ho_DynThreshRegion3, hv_Surf_Dyn3CloseH);
      }
      SelectShape(ho_DynThreshRegion3, &ho_DynThreshRegion3, "area", "and", hv_Surf_Dyn3SizePixS, 
          hv_AREA_MAX);
      //GetRemainRegion (DynThreshRegion3, RegionReduced, AlreadySeg, ProductRegion, DynThreshRegion, DynThreshRegion3, AlreadySeg, RegionReduced, DynThreshRegion)
      Difference(ho_DynThreshRegion3, ho_DynThreshRegionALL, &ho_DynThreshRegion3
          );
      CountObj(ho_DynThreshRegion3, &hv_ErrNumber3);
      ConcatObj(ho_DynThreshRegionALL, ho_DynThreshRegion3, &ho_DynThreshRegionALL
          );
    }
    else
    {
    ...
    if (0 != (hv_tempThreshArea>0))
    {
      if (0 != (hv_Surf_Dyn4CloseW>0))
      {
        ClosingCircle(ho_DynThreshRegion4, &ho_DynThreshRegion4, hv_Surf_Dyn4CloseW);
      }
      Connection(ho_DynThreshRegion4, &ho_DynThreshRegion4);
      RemoveNoiseRegion(ho_DynThreshRegion4, &ho_DynThreshRegion4, "n_48");
      if (0 != (hv_Surf_Dyn4CloseH>0))
      {
        ClosingCircle(ho_DynThreshRegion4, &ho_DynThreshRegion4, hv_Surf_Dyn4CloseH);
      }
      SelectShape(ho_DynThreshRegion4, &ho_DynThreshRegion4, "area", "and", hv_Surf_Dyn4SizePixS, 
          hv_AREA_MAX);
      //GetRemainRegion (DynThreshRegion4, RegionReduced, AlreadySeg, ProductRegion, DynThreshRegion, DynThreshRegion4, AlreadySeg, RegionReduced, DynThreshRegion)
      Difference(ho_DynThreshRegion4, ho_DynThreshRegionALL, &ho_DynThreshRegion4
          );
      CountObj(ho_DynThreshRegion4, &hv_ErrNumber4);
      ConcatObj(ho_DynThreshRegionALL, ho_DynThreshRegion4, &ho_DynThreshRegionALL
          );
    }
    else
    {
  ...
  //
  AreaCenter(ho_DynThreshRegionALL, &hv_Area, &hv_Row, &hv_Column);
  TupleSortIndex(hv_Area, &hv_Indices);
  if (0 != (((hv_Indices.TupleLength())-hv_MaxReturnNum)>0))
  {
    hv_AreaArray = hv_Indices.TupleSelectRange((hv_Indices.TupleLength())-hv_MaxReturnNum,(hv_Indices.TupleLength())-1);
  }
  else
  {
    hv_AreaArray = hv_Indices.TupleSelectRange(0,(hv_Indices.TupleLength())-1);
  }
  SelectObj(ho_DynThreshRegionALL, &ho_DynThreshRegionALL_Temp, hv_AreaArray+1);
  Connection(ho_DynThreshRegionALL_Temp, &ho_DynThreshRegionALL);
  CountObj(ho_DynThreshRegionALL, &hv_Number);
  (*hvec_FeatureVals).Clear();
  {
  HTuple end_val219 = hv_Number;
  HTuple step_val219 = 1;
  for (hv_Index=1; hv_Index.Continue(end_val219, step_val219); hv_Index += step_val219)
  {
    ////逐一选中区域进行分析
    SelectObj(ho_DynThreshRegionALL, &ho_ObjForAsses, hv_Index);
    hv_ErrTypeIndex = 0;
    if (0 != (HTuple(hv_AreaArray[hv_Index-1])<hv_ErrNumber1))
    {
      hv_ErrTypeIndex = 0;
    }
    else if (0 != (HTuple(hv_AreaArray[hv_Index-1])<(hv_ErrNumber1+hv_ErrNumber2)))
    {
      hv_ErrTypeIndex = 1;
    }
    else if (0 != (HTuple(hv_AreaArray[hv_Index-1])<((hv_ErrNumber1+hv_ErrNumber2)+hv_ErrNumber3)))
    {
      hv_ErrTypeIndex = 2;
    }
    else
    ...
    //CalculateFeatures (ObjForAsses, Image, DynThreshRegion1, DynThreshRegion2, DynThreshRegion3, DynThreshRegion4, ProdR1, ProdR2, ProdC1, ProdC2, MessageHandle, FeatureVal)
    hv_Score = 0;
	
    CalculateFeatures_SurfaceNoClass(ho_ObjForAsses, ho_Image, ho_DynThreshRegion1, 
        ho_DynThreshRegion2, ho_DynThreshRegion3, ho_DynThreshRegion4, hv_ProdR1, 
        hv_ProdR2, hv_ProdC1, hv_ProdC2, hv_ErrTypeIndex, hv_MessageHandle, &hv_Score, 
        &hvec_FeatureVal);
    (*hvec_FeatureVals).Insert(hv_Index-1,hvec_FeatureVal);
  }
  }
  if (0 != (hv_ShowHide.TupleAnd(hv_Number>1)))
  {
    try
    {
      //disp_obj (DynThreshRegionALL, WindowHandle)
    }
    // catch (Exception) 
    catch (HalconCpp::HException &HDevExpDefaultException)
    {
    ...
    HTupleVector/*{eTupleVector,Dim=1}*/ *hvec_FeatureVals)
{
  ...
  // Local control variables
  HTuple  hv_Features_retResults_ifTag, hv_errR1;
  HTuple  hv_errC1, hv_errR2, hv_errC2, hv_imgR1, hv_imgR2;
  HTuple  hv_imgC1, hv_imgC2, hv_subSet1, hv_subSet2, hv_subSet3;
  HTuple  hv_subSet4, hv_DEEP_REGION, hv_SHALLOW_REGION, hv_DEEP_REGION2;
  HTuple  hv_SHALLOW_REGION2, hv_Features_properDynThresh;
  ...
  HTuple  hv_Surf_Dyn1SizePixL, hv_Surf_Dyn1SizePixM, hv_Surf_Dyn1SizePixS;
  ...
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixL", &hv_Surf_Dyn1SizePixL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixM", &hv_Surf_Dyn1SizePixM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixS", &hv_Surf_Dyn1SizePixS);
  //
  hv_retClass = "DEFECT";
  ...
  {
    //
    if (0 != (hv_Features_errAreaPix>=hv_Surf_Dyn1SizePixL))
    {
      hv_retClass = "DEEP_L";
      //Features_retResults_ifTag := Surf_IfDeepLTag
    }
    else if (0 != (hv_Features_errAreaPix>=hv_Surf_Dyn1SizePixM))
    {
      hv_retClass = "DEEP_M";
    }
    else if (0 != (hv_Features_errAreaPix>=hv_Surf_Dyn1SizePixS))
    {
      hv_retClass = "DEEP_S";
    }
  }
...
void InitSurCheck (HTuple hv_MessageHandle, HTuple hv_Width, HTuple hv_Height)
{
  ...
  HTuple  hv_CurrentScheme, hv_SystemPath, hv_val, hv_NoClassErrType;
  HTuple  hv_Errtemp, hv_LabelNames, hv_AlarmNames, hv_str_DynMode1;
  HTuple  hv_str_DynMode2, hv_str_DynMode, hv_ImgLeftOffset;
  HTuple  hv_ImgRightOffset, hv_key_exist, hv_RectDrawID;
  ...
  //[缺陷类型]
  //
  hv_NoClassErrType = HTuple();

  load_config(hv_CurrentScheme, "缺陷类型", "0", "黑点", &hv_Errtemp);
  TupleUnion(hv_NoClassErrType, hv_Errtemp, &hv_NoClassErrType);
  load_config(hv_CurrentScheme, "缺陷类型", "1", "亮斑", &hv_Errtemp);
  TupleUnion(hv_NoClassErrType, hv_Errtemp, &hv_NoClassErrType);
  load_config(hv_CurrentScheme, "缺陷类型", "2", "异物", &hv_Errtemp);
  TupleUnion(hv_NoClassErrType, hv_Errtemp, &hv_NoClassErrType);
  load_config(hv_CurrentScheme, "缺陷类型", "3", "划伤", &hv_Errtemp);
  TupleUnion(hv_NoClassErrType, hv_Errtemp, &hv_NoClassErrType);
  SetMessageTuple(hv_MessageHandle, "NoClassErrType", hv_NoClassErrType);
  load_config(hv_CurrentScheme, "缺陷类型", "缺陷小类", "小|中|大", &hv_Errtemp);
  TupleSplit(hv_Errtemp, "|", &hv_Errtemp);
  SetMessageTuple(hv_MessageHandle, "ErrSmallClass", hv_Errtemp);

  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷类型', '0', 'NoClassErrType', '黑点', Errtemp)
  //tuple_union (NoClassErrType, Errtemp, NoClassErrType)
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷类型', '1', 'NoClassErrType', '亮斑', NoClassErrType)
  //tuple_union (NoClassErrType, Errtemp, NoClassErrType)
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷类型', '2', 'NoClassErrType', '异物', NoClassErrType)
  //tuple_union (NoClassErrType, Errtemp, NoClassErrType)
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷类型', '3', 'NoClassErrType', '划伤', NoClassErrType)
  //tuple_union (NoClassErrType, Errtemp, NoClassErrType)
  //[打标报警]
  ...
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynMode1", "Surf_DynMode1", 1, &hv_val);
  ...
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn1CloseW", "Surf_Dyn1CloseW", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn1CloseH", "Surf_Dyn1CloseH", 30, &hv_val);
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynThresh2", "Surf_DynThresh2", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynMode2", "Surf_DynMode2", 0, &hv_val);
  switch (hv_val[0].I())
  {
  case 0:
  ...
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn2CloseW", "Surf_Dyn2CloseW", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn2CloseH", "Surf_Dyn2CloseH", 30, &hv_val);
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynThresh3", "Surf_DynThresh3", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynMode3", "Surf_DynMode3", 1, &hv_val);
  switch (hv_val[0].I())
  {
  case 0:
  ...
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn3CloseW", "Surf_Dyn3CloseW", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn3CloseH", "Surf_Dyn3CloseH", 30, &hv_val);
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynThresh4", "Surf_DynThresh4", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "DynMode4", "Surf_DynMode4", 0, &hv_val);
  switch (hv_val[0].I())
  {
  case 0:
    hv_str_DynMode = "light";
  ...
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn4CloseW", "Surf_Dyn4CloseW", 20, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
      "Dyn4CloseH", "Surf_Dyn4CloseH", 30, &hv_val);
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷识别", 
  ...
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷尺寸', '动态阈值1缺陷阈值', 'Surf_Thresh_Dyn1', 21, val)
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值1小像素", "Surf_Dyn1SizePixS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值1中像素", "Surf_Dyn1SizePixM", 55, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值1大像素", "Surf_Dyn1SizePixL", 65, &hv_val);
  //
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷尺寸', '动态阈值2缺陷阈值', 'Surf_Thresh_Dyn2', 22, val)
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值2小像素", "Surf_Dyn2SizePixS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值2中像素", "Surf_Dyn2SizePixM", 60, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值2大像素", "Surf_Dyn2SizePixL", 100, &hv_val);
  //
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷尺寸', '动态阈值3缺陷阈值', 'Surf_Thresh_Dyn3', 21, val)
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值3小像素", "Surf_Dyn3SizePixS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值3中像素", "Surf_Dyn3SizePixM", 55, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值3大像素", "Surf_Dyn3SizePixL", 65, &hv_val);
  //
  //load_config_set_message_Number (MessageHandle, CurrentScheme, '缺陷尺寸', '动态阈值4缺陷阈值', 'Surf_Thresh_Dyn4', 22, val)
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值4小像素", "Surf_Dyn4SizePixS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值4中像素", "Surf_Dyn4SizePixM", 60, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷尺寸", 
      "动态阈值4大像素", "Surf_Dyn4SizePixL", 100, &hv_val);
  //
  ////[缺陷评分]
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值1小评分", "Surf_Dyn1ScoreS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值1中评分", "Surf_Dyn1ScoreM", 2, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值1大评分", "Surf_Dyn1ScoreL", 3, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值2小评分", "Surf_Dyn2ScoreS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值2中评分", "Surf_Dyn2ScoreM", 2, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值2大评分", "Surf_Dyn2ScoreL", 3, &hv_val);
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值3小评分", "Surf_Dyn3ScoreS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值3中评分", "Surf_Dyn3ScoreM", 2, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值3大评分", "Surf_Dyn3ScoreL", 3, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值4小评分", "Surf_Dyn4ScoreS", 1, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值4中评分", "Surf_Dyn4ScoreM", 2, &hv_val);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
      "动态阈值4大评分", "Surf_Dyn4ScoreL", 3, &hv_val);
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "缺陷评分", 
  ...
  //
  //
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "检测区域", 
      "左边界", "ImgLeftOffset", 0, &hv_ImgLeftOffset);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "检测区域", 
      "右边界", "ImgRightOffset", 3746, &hv_ImgRightOffset);
  GetMessageParam(hv_MessageHandle, "key_exists", "RectDrawID", &hv_key_exist);
  if (0 != (hv_key_exist==0))
  {
    CreateDrawingObjectRectangle1(0, hv_ImgLeftOffset, hv_Height, hv_ImgRightOffset, 
        &hv_RectDrawID);
    SetMessageTuple(hv_MessageHandle, "RectDrawID", hv_RectDrawID);
    SetMessageTuple(hv_MessageHandle, "RectDrawID_CloseClear", "clear_drawing_object");
  }



  //gen_rectangle1 (hCheckRegion, 0, ImgLeftOffset, ImageH, ImgRightOffset)
  //set_message_obj (hCheckRegion, MessageHandle, 'hCheckRegion')
  //
...
void Init_FloorCheck (HTuple hv_CurrentScheme, HTuple hv_CheckParam, HTuple hv_MessageHandle)
{
  ...
  // Local control variables
  HTuple  hv_CameraIndex, hv_ImageW, hv_ImageH;
  HTuple  hv_DirName, hv_CurrentSchemePath, hv_SystemPath;
  HTuple  hv_ThresholdValUse, hv_ThresholdValMin, hv_ThresholdValMax;
  HTuple  hv_TransverseExtend;
  ...
  //产品阈值
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "产品背景", 
      "阈值启用", "ThresholdValUse", 1, &hv_ThresholdValUse);
  if (0 != (hv_ThresholdValUse==1))
  {
    load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "产品背景", 
        "阈值Min", "ThresholdValMin", 80, &hv_ThresholdValMin);
    load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "产品背景", 
        "阈值Max", "ThresholdValMax", 255, &hv_ThresholdValMax);
  }
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, "裁剪区域", 
      "横向扩展", "TransverseExtend", 5, &hv_TransverseExtend);
...
void Init_Python_Memory (HTuple hv_TestPlanIni, HTuple hv_CheckParam, HTuple hv_MessageHandle)
{
  ...
  HTuple  hv_PyFileIndex, hv_Py_ResultUse, hv_PyImgSave, hv_PyImgSaveDir;
  HTuple  hv_UsePythonCheck, hv_FileName_ImageInfo, hv_FileName_ImageInfo_handle;
  ...
  load_config_set_message_Number(hv_MessageHandle, hv_TestPlanIni, hv_CheckParam, 
      "Py图像存储", "PyImgSave", 1, &hv_PyImgSave);
  load_config_set_message_Number(hv_MessageHandle, hv_TestPlanIni, hv_CheckParam, 
      "Py图像存储目录", "PyImgSaveDir", "D:/PyImg", &hv_PyImgSaveDir);
  load_config_set_message_Number(hv_MessageHandle, hv_TestPlanIni, hv_CheckParam, 
...
void InitGlobalOnce ()
{
  ...
  HTuple  hv_SystemPath, hv_filemappingMax;
    ...
    HTuple hv_ImageInfos, HTuple *hv_ResultVal)
{
  ...
  HTuple  hv_PyImgSave, hv_PyImgSaveDir, hv_MSecond, hv_Second;
  HTuple  hv_Minute, hv_Hour, hv_Day, hv_YDay, hv_Month, hv_Year;
  HTuple  hv_Exception, hv_FileName_ResultInfo_handle, hv_ImageResultInfo;
  ...
  if (0 != (hv_UsePythonCheck!=0))
  {
    ...
    try
    {
      GetMessageTuple(hv_MessageHandle, "PyImgSave", &hv_PyImgSave);
      if (0 != (hv_PyImgSave==1))
      {
        GetMessageTuple(hv_MessageHandle, "PyImgSaveDir", &hv_PyImgSaveDir);
        GetImageTime((*ho_ImageResult), &hv_MSecond, &hv_Second, &hv_Minute, &hv_Hour, 
            &hv_Day, &hv_YDay, &hv_Month, &hv_Year);

        SaveImage(ho_Image, (((((((hv_PyImgSaveDir+"/")+hv_Year)+hv_Month)+hv_Day)+hv_Minute)+hv_Second)+hv_MSecond)+".jpg");
      }
    }
    // catch (Exception) 
    catch (HalconCpp::HException &HDevExpDefaultException)
...
void SaveImageThread (HObject ho_Image, HTuple hv_Format, HTuple hv_fillcolor, HTuple hv_filename)
{
  ...
  WriteImage(ho_Image, hv_Format, hv_fillcolor, hv_filename);
  return;
}
    ...
    HTuple hv_MessageHandle)
{
  ...
  GetMessageTuple(hv_MessageHandle, "DiameterVal", &hv_DiameterVal);
  GetMessageTuple(hv_MessageHandle, "Diameter_offset", &hv_Diameter_offset);
    ...
    HTuple hv_MessageHandle)
{
  ...
  HTuple  hv_ClawRectVal, hv_GuideCutOffRuleR, hv_DiameterMeasureW_mm;
  HTuple  hv_GuidepinMeasureW_mm, hv_DiameterVal, hv_Diameter_offset;
  HTuple  hv_DiameterMinMax, hv_GuidepinVal, hv_Guidepin_offset;
  HTuple  hv_GuidepinMinMax;
  ...
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, HTuple(hv_CheckParam[0]), 
      "产品分割线R", "GuideCutOffRuleR", 1024, &hv_GuideCutOffRuleR);
  ...
  GenRectangle1(&ho_DiameterRect1, HTuple(hv_ClawRectVal[0]), HTuple(hv_ClawRectVal[1])-hv_DiameterMeasureW_mm, 
      hv_GuideCutOffRuleR, HTuple(hv_ClawRectVal[1]));
  GenRectangle1(&ho_DiameterRect2, hv_GuideCutOffRuleR, HTuple(hv_ClawRectVal[1])-hv_DiameterMeasureW_mm, 
      HTuple(hv_ClawRectVal[2]), HTuple(hv_ClawRectVal[1]));
  ...
  GenRectangle1(&ho_GuideRect1, HTuple(hv_ClawRectVal[0]), HTuple(hv_ClawRectVal[3]), 
      hv_GuideCutOffRuleR, HTuple(hv_ClawRectVal[3])+hv_GuidepinMeasureW_mm);
  GenRectangle1(&ho_GuideRect2, hv_GuideCutOffRuleR, HTuple(hv_ClawRectVal[3]), HTuple(hv_ClawRectVal[2]), 
      HTuple(hv_ClawRectVal[3])+hv_GuidepinMeasureW_mm);
...
void Init_TopSurfaceDetection (HTuple hv_CurrentScheme, HTuple hv_lsCheckParam, HTuple hv_MessageHandle)
{
  ...
  GetMessageTuple(hv_MessageHandle, "GuidepinVal", &hv_GuidepinVal);
  GetMessageTuple(hv_MessageHandle, "Guidepin_offset", &hv_Guidepin_offset);
    ...
    HTuple hv_MessageHandle)
{
  ...
  HTuple  hv_Index, hv_OCRval, hv_val, hv_OCRDirection, hv_shmfile;
  HTuple  hv_shmModelId, hv_Row1, hv_Column1, hv_Row2, hv_Column2;
  HTuple  hv_shmLen, hv_shmps, hv_dfmfile, hv_dfmModelId;
  HTuple  hv_dfmLen, hv_dfmps, hv_Rectval, hv_TextModel, hv_TextModel_polarity;
  HTuple  hv_OcrHandle;

  //获得预处理图像的分割区域
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, HTuple(hv_CheckParam[0]), 
      "图像增强", "ImageAdd", 0.5, &hv_ImageAdd);
  GetMessageTuple(hv_MessageHandle, "ProductHeight", &hv_ProductHeight);
  load_config_set_message_Number(hv_MessageHandle, hv_CurrentScheme, HTuple(hv_CheckParam[0]), 
  ...
  {
  HTuple end_val11 = (hv_OCRSigns.TupleLength())-1;
  HTuple step_val11 = 1;
  for (hv_Index=0; hv_Index.Continue(end_val11, step_val11); hv_Index += step_val11)
  {

    GetMessageTuple(hv_MessageHandle, HTuple(hv_OCRSigns[hv_Index])+"_val", &hv_OCRval);
    ...
    load_config_set_message_Str(hv_MessageHandle, hv_CurrentScheme, HTuple(hv_CheckParam[0]), 
        ...
        "dfm", &hv_val);
    GetMessageTuple(hv_MessageHandle, HTuple(hv_OCRSigns[hv_Index])+"_Direction", 
        &hv_OCRDirection);
    load_config_set_message_Str(hv_MessageHandle, hv_CurrentScheme, HTuple(hv_CheckParam[0]), 
    ...
    GetMessageTuple(hv_MessageHandle, "TextModel_polarity", &hv_TextModel_polarity);
    SetTextModelParam(hv_TextModel, "polarity", hv_TextModel_polarity);
    ...
    HTuple *hv_ResultInfo, HTuple *hv_WindowInfo, HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_FeatureVals)
{
  ...
  HObject  ho_GRegion1, ho_GRegion2, ho_Regions, ho_ConnectedRegions;
  HObject  ho_SelectedRegions, ho_SelectedRegions1, ho_Rectangle1;
  HObject  ho_Rectangle2, ho_ImageReduced1, ho_ImageReduced2;
  ...
  HTuple  hv_GuideCutOffRuleR, hv_Row11, hv_Column11, hv_Row21;
  HTuple  hv_Column21, hv_Row12, hv_Column12, hv_Row22, hv_Column22;
  HTuple  hv_Row13, hv_Column13, hv_Row23, hv_Column23, hv_chaval;
  HTuple  hv_GuidepinHeightoffset;
  ...
  if (0 != (HTuple((*hv_ResultInfo)==HTuple("OK")).TupleOr((*hv_ResultInfo)==HTuple(""))))
  {
    //导针分割线R

    GetMessageTuple(hv_MessageHandle, "GuideCutOffRuleR", &hv_GuideCutOffRuleR);
    //gen_rectangle1 (Rectangle, GuideCutOffRuleR-20, GColumn2, GuideCutOffRuleR+20, GColumn2)
    Threshold(ho_Image, &ho_Regions, 255, 255);
    Connection(ho_Regions, &ho_ConnectedRegions);
    SelectShape(ho_ConnectedRegions, &ho_SelectedRegions, "column2", "and", hv_GColumn1, 
        hv_Width);
    SelectShapeStd(ho_SelectedRegions, &ho_SelectedRegions1, "max_area", 100);
    SmallestRectangle1(ho_SelectedRegions1, &hv_Row11, &hv_Column11, &hv_Row21, &hv_Column21);

    GenRectangle1(&ho_Rectangle1, hv_Row1, hv_Column11-10, hv_GuideCutOffRuleR, hv_Column21-20);
    GenRectangle1(&ho_Rectangle2, hv_GuideCutOffRuleR, hv_Column11-10, hv_Row2, hv_Column21-20);
    ReduceDomain(ho_Image, ho_Rectangle1, &ho_ImageReduced1);
    ReduceDomain(ho_Image, ho_Rectangle2, &ho_ImageReduced2);
    Threshold(ho_ImageReduced1, &ho_Region1, 0, 160);
    Threshold(ho_ImageReduced2, &ho_Region2, 0, 160);
    SmallestRectangle1(ho_Region1, &hv_Row12, &hv_Column12, &hv_Row22, &hv_Column22);
    SmallestRectangle1(ho_Region2, &hv_Row13, &hv_Column13, &hv_Row23, &hv_Column23);

    hv_chaval = ((hv_Column22-hv_Column23).TupleAbs())*hv_resolutionratio;

    GetMessageTuple(hv_MessageHandle, "GuidepinHeightoffset", &hv_GuidepinHeightoffset);
    (*hv_WindowInfo) = (*hv_WindowInfo).TupleConcat(((("导针高度差:"+hv_chaval)+"[")+hv_GuidepinHeightoffset)+"]");
    if (0 != (hv_chaval>hv_GuidepinHeightoffset))
    {
      (*hv_ResultInfo) = "NG";
    }
  }
    ...
    HTuple *hv_ResultInfo, HTuple *hv_WindowInfo, HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_FeatureVals)
{
  ...
  hv_msg = "Ai_信息";
  {
  HTuple end_val45 = (hv_PyImageInfo.TupleLength())-1;
  HTuple step_val45 = 1;
    ...
    HTuple *hv_isFindOCR, HTuple *hv_WindowInfo)
{
  ...
  // Local iconic variables
  HObject  ho_ModelContours, ho_TransContours, ho_Region;
  HObject  ho_RegionUnion, ho_Rectangle2, ho_ImageReduced1;
  HObject  ho_ImagePart, ho_ImageRotate1, ho_ContoursAffinTrans;
  HObject  ho_Retangle1, ho_Rectangle1, ho_Rectangle, ho_ImageRotate;
  ...
  if (0 != (hv_shmdfm==HTuple("dfm")))
  {
    ...
    FindPlanarUncalibDeformableModel(ho_ImageReduced, hv_ModelId, HTuple(0).TupleRad(), 
        HTuple(360).TupleRad(), 1, 1, 1, 1, 0.5, 10, 0.5, 4, 0.75, "subpixel", "least_squares_very_high", 
    ...
    {
    HTuple end_val17 = (hv_ModelScore.TupleLength())-1;
    HTuple step_val17 = 1;
    for (hv_MatchingObjIdx=0; hv_MatchingObjIdx.Continue(end_val17, step_val17); hv_MatchingObjIdx += step_val17)
    {
      ...
      if (0 != (hv_OCRDirection==0))
      {
        ...
        Show_Hide(ho_Rectangle2, 1, hv_WindowHandle);
  ...
  else if (0 != (hv_shmdfm==HTuple("shm")))
  {
    ...
    {
    HTuple end_val95 = (hv_ModelAngle.TupleLength())-1;
    HTuple step_val95 = 1;
    for (hv_Index2=0; hv_Index2.Continue(end_val95, step_val95); hv_Index2 += step_val95)
    {
    ...
    {
    HTuple end_val123 = (hv_degModelAngle.TupleLength())-1;
    HTuple step_val123 = 1;
    for (hv_Index2=0; hv_Index2.Continue(end_val123, step_val123); hv_Index2 += step_val123)
    {
    ...
    HTupleVector/*{eTupleVector,Dim=1}*/ *hvec_FeatureVals)
{
  ...
  // Local iconic variables
  HObject  ho_regionDyn, ho_Rectangle;

  // Local control variables
  HTuple  hv_Resolution_H, hv_Resolution_W, hv_ErrImgSaveWidth;
  HTuple  hv_ErrImgSaveHeight, hv_NoClassErrType, hv_NoClassErrType_Small;
  HTuple  hv_Surf_Calib, hv_Surf_Calib2, hv_Surf_Dyn1SizePixL;
  HTuple  hv_Surf_Dyn1SizePixM, hv_Surf_Dyn1SizePixS, hv_Surf_Dyn2SizePixL;
  HTuple  hv_Surf_Dyn2SizePixM, hv_Surf_Dyn2SizePixS, hv_Surf_Dyn3SizePixL;
  HTuple  hv_Surf_Dyn3SizePixM, hv_Surf_Dyn3SizePixS, hv_Surf_Dyn4SizePixL;
  HTuple  hv_Surf_Dyn4SizePixM, hv_Surf_Dyn4SizePixS, hv_Surf_Dyn1ScoreS;
  HTuple  hv_Surf_Dyn1ScoreM, hv_Surf_Dyn1ScoreL, hv_Surf_Dyn2ScoreS;
  HTuple  hv_Surf_Dyn2ScoreM, hv_Surf_Dyn2ScoreL, hv_Surf_Dyn3ScoreS;
  HTuple  hv_Surf_Dyn3ScoreM, hv_Surf_Dyn3ScoreL, hv_Surf_Dyn4ScoreS;
  HTuple  hv_Surf_Dyn4ScoreM, hv_Surf_Dyn4ScoreL, hv_regionArea;
  HTuple  hv_errCenterR, hv_errCenterC, hv_errR1, hv_errC1;
  HTuple  hv_errR2, hv_errC2, hv_Features_regionWidthPix;
  HTuple  hv_Features_regionHeightPix, hv_errDiam, hv_Width;
  HTuple  hv_Height, hv_Features_errAreaPix, hv_Features_retResults_rectvals;
  HTuple  hv_Features_retResults_ErrorArea, hv_Features_retResults_Diameter;
  HTuple  hv_Features_retResults_x, hv_Features_retResults_y;
  HTuple  hv_ID;

  GetMessageTuple(hv_MessageHandle, "Resolution_H", &hv_Resolution_H);
  GetMessageTuple(hv_MessageHandle, "Resolution_W", &hv_Resolution_W);

  GetMessageTuple(hv_MessageHandle, "ErrImgSaveWidth", &hv_ErrImgSaveWidth);
  GetMessageTuple(hv_MessageHandle, "ErrImgSaveHeight", &hv_ErrImgSaveHeight);
  GetMessageTuple(hv_MessageHandle, "NoClassErrType", &hv_NoClassErrType);
  GetMessageTuple(hv_MessageHandle, "ErrSmallClass", &hv_NoClassErrType_Small);
  GetMessageTuple(hv_MessageHandle, "Surf_Calib", &hv_Surf_Calib);
  GetMessageTuple(hv_MessageHandle, "Surf_Calib2", &hv_Surf_Calib2);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixL", &hv_Surf_Dyn1SizePixL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixM", &hv_Surf_Dyn1SizePixM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1SizePixS", &hv_Surf_Dyn1SizePixS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2SizePixL", &hv_Surf_Dyn2SizePixL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2SizePixM", &hv_Surf_Dyn2SizePixM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2SizePixS", &hv_Surf_Dyn2SizePixS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3SizePixL", &hv_Surf_Dyn3SizePixL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3SizePixM", &hv_Surf_Dyn3SizePixM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3SizePixS", &hv_Surf_Dyn3SizePixS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4SizePixL", &hv_Surf_Dyn4SizePixL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4SizePixM", &hv_Surf_Dyn4SizePixM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4SizePixS", &hv_Surf_Dyn4SizePixS);

  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1ScoreS", &hv_Surf_Dyn1ScoreS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1ScoreM", &hv_Surf_Dyn1ScoreM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn1ScoreL", &hv_Surf_Dyn1ScoreL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2ScoreS", &hv_Surf_Dyn2ScoreS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2ScoreM", &hv_Surf_Dyn2ScoreM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn2ScoreL", &hv_Surf_Dyn2ScoreL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3ScoreS", &hv_Surf_Dyn3ScoreS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3ScoreM", &hv_Surf_Dyn3ScoreM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn3ScoreL", &hv_Surf_Dyn3ScoreL);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4ScoreS", &hv_Surf_Dyn4ScoreS);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4ScoreM", &hv_Surf_Dyn4ScoreM);
  GetMessageTuple(hv_MessageHandle, "Surf_Dyn4ScoreL", &hv_Surf_Dyn4ScoreL);
  ...
  CopyObj(ho_InputRegion, &ho_regionDyn, 1, 1);
  AreaCenter(ho_regionDyn, &hv_regionArea, &hv_errCenterR, &hv_errCenterC);
  SmallestRectangle1(ho_regionDyn, &hv_errR1, &hv_errC1, &hv_errR2, &hv_errC2);
  hv_Features_regionWidthPix = (hv_errC2-hv_errC1)+1;
  hv_Features_regionHeightPix = (hv_errR2-hv_errR1)+1;
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_regionWidthPix").TupleConcat(hv_Features_regionWidthPix))));
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_regionHeightPix").TupleConcat(hv_Features_regionHeightPix))));
  ////取较大值作为直径
  hv_errDiam = ((((hv_errR2-hv_errR1)+1)*hv_Resolution_H).TupleConcat(((hv_errC2-hv_errC1)+1)*hv_Resolution_W)).TupleMax();
  //获得瑕疵小图存储范围
  GenRectangle1(&ho_Rectangle, hv_errCenterR-(hv_ErrImgSaveHeight/2), hv_errCenterC-(hv_ErrImgSaveWidth/2), 
      hv_errCenterR+(hv_ErrImgSaveHeight/2), hv_errCenterC+(hv_ErrImgSaveWidth/2));
  if (0 != ((hv_errCenterR-(hv_ErrImgSaveHeight/2))<0))
  {
    MoveRegion(ho_Rectangle, &ho_Rectangle, (hv_ErrImgSaveHeight/2)-hv_errCenterR, 
        0);
  }
  if (0 != ((hv_errCenterC-(hv_ErrImgSaveWidth/2))<0))
  {
    MoveRegion(ho_Rectangle, &ho_Rectangle, 0, (hv_ErrImgSaveWidth/2)-hv_errCenterC);
  }

  GetImageSize(ho_Image, &hv_Width, &hv_Height);
  if (0 != ((hv_errCenterR+(hv_ErrImgSaveHeight/2))>hv_Height))
  {
    MoveRegion(ho_Rectangle, &ho_Rectangle, (hv_Height-hv_errCenterR)-(hv_ErrImgSaveHeight/2), 
        0);
  }
  if (0 != ((hv_errCenterC+(hv_ErrImgSaveWidth/2))>hv_Width))
  {
    MoveRegion(ho_Rectangle, &ho_Rectangle, 0, (hv_Width-hv_errCenterC)-(hv_ErrImgSaveWidth/2));
  }
  SmallestRectangle1(ho_Rectangle, &hv_errR1, &hv_errC1, &hv_errR2, &hv_errC2);
  ////缺陷面积及中心位置
  hv_Features_errAreaPix = hv_regionArea;
  hv_Features_retResults_rectvals.Clear();
  hv_Features_retResults_rectvals.Append(hv_errC1);
  hv_Features_retResults_rectvals.Append(hv_errR1);
  hv_Features_retResults_rectvals.Append(hv_errC2);
  ...
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_errAreaPix").TupleConcat(hv_Features_errAreaPix))));
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_retResults_rectvals").TupleConcat(hv_Features_retResults_rectvals))));
  hv_Features_retResults_ErrorArea = ((hv_regionArea*hv_Resolution_W)*hv_Resolution_H)*hv_Surf_Calib2;
  hv_Features_retResults_Diameter = hv_errDiam*hv_Surf_Calib;
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_retResults_ErrorArea").TupleConcat(hv_Features_retResults_ErrorArea))));
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_retResults_Diameter").TupleConcat(hv_Features_retResults_Diameter))));
  ...
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_retResults_y").TupleConcat(hv_Features_retResults_y))));
  //
  TupleString(hv_ErrTypeIndex, "#", &hv_ID);
  (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),HTupleVector(HTuple(HTuple("Features_retResults_ErrortypeID").TupleConcat(hv_ID))));
    ...
    HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_outFeatureVals)
{
  ...
  HObject  ho_Rectangle, ho_ImageReduced, ho_ImageResult;

  // Local control variables
  HTuple  hv_ThresholdValUse, hv_Width, hv_Height;
  HTuple  hv_ThresholdValMin, hv_ThresholdValMax, hv_Row1;
  HTuple  hv_Column1, hv_Row2, hv_Column2, hv_TransverseExtend;
  HTuple  hv_Width1, hv_Height1, hv_PyImageInfo, hv_PyThreadID;
  HTuple  hv_PyResultVal, hv_PyResultInfo, hv_PyWindowInfo;
  HTuple  hv_msg, hv_Index;
  HTupleVector  hvec_PyFeatureVals(2);
  ...
  //产品阈值
  //
  GetMessageTuple(hv_MessageHandle, "ThresholdValUse", &hv_ThresholdValUse);
  if (0 != (hv_ThresholdValUse==1))
  {
    GetImageSize(ho_PreprocessImage, &hv_Width, &hv_Height);
    GetMessageTuple(hv_MessageHandle, "ThresholdValMin", &hv_ThresholdValMin);
    GetMessageTuple(hv_MessageHandle, "ThresholdValMax", &hv_ThresholdValMax);
    Threshold(ho_PreprocessImage, &ho_Region, hv_ThresholdValMin, hv_ThresholdValMax);
    Connection(ho_Region, &ho_ConnectedRegions);
    SelectShapeStd(ho_ConnectedRegions, &ho_SelectedRegions, "max_area", 100);
    SmallestRectangle1(ho_SelectedRegions, &hv_Row1, &hv_Column1, &hv_Row2, &hv_Column2);

    GetMessageTuple(hv_MessageHandle, "TransverseExtend", &hv_TransverseExtend);
    GenRectangle1(&ho_Rectangle, 0, hv_Column1-hv_TransverseExtend, hv_Height, hv_Column2+hv_TransverseExtend);
    DispObj(ho_Rectangle, hv_WindowHandle);
    ReduceDomain(ho_PreprocessImage, ho_Rectangle, &ho_ImageReduced);
    CropDomain(ho_ImageReduced, &ho_PreprocessImage);

  }
  else
  {
    CopyImage(ho_PreprocessImage, &ho_PreprocessImage);
  }
  GetImageSize(ho_PreprocessImage, &hv_Width1, &hv_Height1);

  hv_PyImageInfo = HTuple();
  // Create a thread instance
  hcppthread_handle = new HDevThread(hcppthread_context,
              (void*)HDevExportCpp::_hcppthread_Py_Memory_Check,7,2);
  // Set thread procedure call arguments 
  hcppthread_handle->SetInputIconicParamObject(0,ho_PreprocessImage);
  hcppthread_handle->SetInputCtrlParamTuple(1,hv_MessageHandle);
  hcppthread_handle->SetInputCtrlParamTuple(2,hv_WindowHandle);
  hcppthread_handle->SetInputCtrlParamTuple(3,hv_ShowHide);
  hcppthread_handle->SetInputCtrlParamTuple(4,hv_Width1);
  ...
  HDevThread::ParJoin(hv_PyThreadID);
  GetPyFeature2(ho_ImageResult, hv_MessageHandle, hv_ShowHide, hv_WindowHandle, hv_PyResultVal, 
      0, hv_Column1, &hv_PyResultInfo, &hv_PyWindowInfo, &hvec_PyFeatureVals);
  (*hv_outResultInfo) = hv_PyResultInfo;
  hv_msg = "Ai_信息";
  {
  HTuple end_val35 = (hv_PyImageInfo.TupleLength())-1;
  HTuple step_val35 = 1;
  ...
  (*hv_outWindowInfo).Clear();
  (*hv_outWindowInfo).Append(hv_msg);
  (*hv_outWindowInfo).Append(hv_PyWindowInfo);
  (*hvec_outFeatureVals) = hvec_PyFeatureVals;
  //
  //
  //
  //
    ...
    HTuple *hv_WindowInfo, HTupleVector/*{eTupleVector,Dim=2}*/ *hvec_FeatureVals)
{
  ...
  HTuple  hv_GrayColumn, hv_Row11, hv_Column11, hv_Row21;
  HTuple  hv_Column21, hv_Diameter, hv_Row1, hv_Column1, hv_Row2;
  HTuple  hv_Column2, hv_Errstring, hv_Py_ResultUse;
  HTupleVector  hvec_FeatureVal(1);
  ...
  else if (0 != (hv_PyResultVal==1))
  {
    ...
    for (hv_GrayIndex=0; hv_GrayIndex.Continue(end_val22, step_val22); hv_GrayIndex += step_val22)
    {
      Threshold(ho_ImageResult, &ho_GrayRegion, HTuple(hv_PyImageGaryPriority[hv_GrayIndex]), 
          HTuple(hv_PyImageGaryPriority[hv_GrayIndex]));
      MoveRegion(ho_GrayRegion, &ho_GrayRegion, hv_R, hv_C);
      AreaCenter(ho_GrayRegion, &hv_GrayArea, &hv_GrayRow, &hv_GrayColumn);
      if (0 != (hv_GrayArea>0))
      {
        if (0 != hv_ShowHide)
        ...
        hvec_FeatureVal.Insert(hvec_FeatureVal.Length(),HTupleVector((HTuple("Features_retResults_ifTag").Append(0))));
        hvec_FeatureVal.Insert(hvec_FeatureVal.Length(),HTupleVector((HTuple("Features_retResults_Alarm").Append(0))));
        TupleString(HTuple(hv_PyImageGaryPriority[hv_GrayIndex]), "#", &hv_Errstring);
        hvec_FeatureVal.Insert(hvec_FeatureVal.Length(),HTupleVector(HTuple(HTuple("Features_retResults_ErrortypeID").TupleConcat(hv_Errstring))));
        //
        (*hvec_FeatureVals).Insert((*hvec_FeatureVals).Length(),hvec_FeatureVal);
        (*hv_ResultInfo) = ((const HTuple&)hv_PyImageGaryTypeName)[hv_GrayIndex];
        //
    ...
    HTuple hv_CameraIndex, HTuple hv_ImagePath)
{
  ...
  OpenWindow(0,0,700,400,0,"","",&hv_WindowHandle);
